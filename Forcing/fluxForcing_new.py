...
def mixing_regime_summary(romsfile, tke_threshold=1e-6):
    ...
    mld_times, mld_indices = compute_mld_density(
        f, i=7, j=6, drho_crit=0.03, return_index=True
    )
...
def csv_to_latex_matrix(
    csv_file, caption="3×3 Mixing Regime Matrix", label="tab:mixing_matrix"
):
    """
    ...
    for i, wind in enumerate(winds):
        for j, cloud in enumerate(clouds):
            ...
            if len(row) == 1:
                ...
                matrix[i][j] = (
                    f"MLD={mld} m\\\\Conv={conv}\\\\Shear={shear}\\\\Stable={stable}"
                )
            else:
                matrix[i][j] = "N/A"
    ...
    for wind in winds:
        ...
        for _, row in df_w.iterrows():
            ...
            if first:
                ...
                latex += f" & {cloud} & {mld} & {conv} & {shear} & {stable} \\\\\n"
        ...
        for wind in winds:
            for cloud in clouds:
                ...
                if os.path.exists(romsfile):
                    summary[(diurnal, cloud, wind)] = mixing_regime_summary(romsfile)
                else:
...
def make_mld_table(summary):
    winds = [5, 10, 15]
    clouds = [0, 50, 100]  # in % #[0.0, 0.5, 1.0]  # in fraction
    print("Cloud cover, Wind, ID, Mean MLD diurnal, ID, Meand MLD avg, Difference")
    for cloud in clouds:
        for wind in winds:
            ...
            print(
                f"{cloud}, {wind}, E{id1:02}, {mean1:5.1f}, E{id2:02}, {mean2:5.1f}, {mean1 - mean2:5.1f}"
            )
...
def make_sst_table(summary):
    winds = [5, 10, 15]
    clouds = [0, 50, 100]  # in % #[0.0, 0.5, 1.0]  # in fraction
    print("Cloud cover, Wind, ID, Mean SST diurnal, ID, Mean SST avg, Difference")
    for cloud in clouds:
        for wind in winds:
            summary_diurnal = summary[(True, cloud, wind)]
            summary_nondiurnal = summary[(False, cloud, wind)]
...
def make_fraction_unstable_table(summary):
    winds = [5, 10, 15]
    clouds = [0, 50, 100]  # in % #[0.0, 0.5, 1.0]  # in fraction
    print("Cloud cover, Wind, ID, Unstable diurnal, ID, Unstable  avg, Difference")
    for cloud in clouds:
        for wind in winds:
            summary_diurnal = summary[(True, cloud, wind)]
            summary_nondiurnal = summary[(False, cloud, wind)]
...
def make_bulkforce_file(
    diurnal=True, sw_amplitude=800.0, cloud=0.0, u_wind=15.0, num_days_with_wind=7
):
    ...
    if diurnal:
        ...
        swrad = np.maximum(
            sw_amplitude_clouds * np.cos(onedayfreq * (timevec - phase_shift)), 0
        ).mean()
...
def move_results(folder):
    ...
    for fname in [
        ...
        log = subprocess.run(
            ["mv", os.path.join(FORCING_FOLDER, fname), os.path.join(folder, fname)],
            capture_output=True,
            text=True,
        ).stdout
        print("Moved ", fname)
    ...
    for subfolder in ["External", "Include"]:
        ...
        for fname in os.listdir(in_folder):
            subprocess.run(
                ["cp", os.path.join(in_folder, fname), os.path.join(out_folder, fname)],
                capture_output=True,
                text=True,
            ).stdout
...
def plot_density_difference_hovmuller(file1, file2, filename=None):
    ...
    # 6. Formatting
    cbar_obj = plt.colorbar(cf, label=r"$\Delta \rho$ [kg/m$^3$]", extend="both")
...
def plot_tke_stability_hovmuller(romsfile, filename=None):
    ...
    cbar.ax.set_yticklabels(
        [r"Convective ($R_i<0$)", r"Shear ($0≤R_i<0.25$)", r"Stable ($R_i≥0.25$)"]
    )
...
def plot_tke_difference_hovmuller(file1, file2, filename=None):
    ...
    # 7. Formatting
    cbar_obj = plt.colorbar(cf, label=r"$\Delta log TKE$ [$m^2/s^2$]", extend="both")
...
def plot_speed_difference_hovmuller(file1, file2, filename=None):
    ...
    # 7. Formatting
    cbar_obj = plt.colorbar(cf, label=r"$\Delta |U|$ [cm/s]", extend="both")
...
def plot_hodograph(romsfile, savefile=False, ext=".png"):
    ...
    for i in range(N):
        ...
        if savefile:
            ...
            plt.savefig(
                os.path.join(folder, "hodograph" + ext), dpi=300, bbox_inches="tight"
            )
        # plt.show()
...
def calculate_roms_cell_volume(file_path: str):
    ...
    try:
        ...
        print(
            f"Shape of the resulting 'volume' array: ({time_size}, {s_rho_size}, {eta_rho_size}, {xi_rho_size})"
        )
        ...
        sample_volume = volume.isel(
            ocean_time=time_step, s_rho=s_layer, eta_rho=eta_index, xi_rho=xi_index
        ).values
...
def robust_time_conversion(time_da: xr.DataArray):
    ...
    time_days_da = xr.DataArray(
        time_days_values, dims=time_da.dims, coords={time_da.dims[0]: time_days_values}
    )
...
def plot_thermodynamic_fluxes(his_file: str, frc_file: str, filename: str = ""):
    ...
    try:
        ...
        q_net_total.plot(
            ax=ax, label="$Q_H$ (Net Total Heat Flux)", color="black", linewidth=3.0
        )
        ...
        ax.set_title(
            f"Thermodynamic Heat Flux Components (Grid Average)\nEquation: {title_eq}"
        )
        ax.set_xlabel("Days")
...
def calculate_model_heat_stats_2D_with_boundary(ds_his, rho0=1025.0, Cp=3985.0):
    ...
    volume_aligned = xr.DataArray(volume.values, coords=temp.coords, dims=temp.dims)
...
def calculate_model_heat_stats_single_point(ds_his, rho0=1025.0, Cp=3985.0):
    ...
    try:
        cell_area = 1.0 / (
            pm.isel(eta_rho=6, xi_rho=7).item() * pn.isel(eta_rho=6, xi_rho=7).item()
        )
        ...
        print(
            "Advarsel: Grid-variablene pm/pn har uventede dimensjoner. Bruker standard 1x1 areal."
        )
        cell_area = 1.0
...
def calculate_forcing_heat_stats_single_point(ds_his):
    ...
    net_flux = (
        ds_his["shflux"].isel(eta_rho=6, xi_rho=7).assign_coords(ocean_time=his_days)
    )  # W/m^2
...
def plot_heat_flux_components(his_file: str, filename=None):
    ...
    lw_net = (
        ds_his["lwrad"].isel(eta_rho=6, xi_rho=7).assign_coords(ocean_time=his_days)
    )
    ...
    latent = (
        ds_his[latname].isel(eta_rho=6, xi_rho=7).assign_coords(ocean_time=his_days)
        if latname in ds_his
        else 0
    )
    sensible = (
        ds_his[senname].isel(eta_rho=6, xi_rho=7).assign_coords(ocean_time=his_days)
    ...
    plt.plot(
        his_days, sensible, label="Sensible (From History)", color="green", alpha=0.6
    )
...
def plot_temp_difference_hovmuller(file1, file2, filename=None):
    ...
    plt.grid(axis="x")
    plt.title(
        f"Temperature Difference: {os.path.basename(file1)} - {os.path.basename(file2)}"
    )
...
def plots():
    ...
    for new_experiment in new_experiments:
        ...
        if True:  # folder.startswith('2025-09-24'):#
            ...
            verify_heat_content(
                romsfile, filename=os.path.join(root, "verify_heat_content" + ext)
            )
...
def compare_results():
    ...
    for experiments in experiments2compare:
        ...
        compare_heat_content(
            folders,
            filename=os.path.join(folders[-1], f"compare_heat_content{txt}.png"),
        )
...
def compare_ncdiff():
    ...
    for experiments in experiments2compare:
        ...
        plot_tke_difference_hovmuller(
            romsfile1,
            romsfile2,
            filename=os.path.join(folder2, f"tke_hovmuller_{difftxt}" + ext),
        )
    ...
    for i, wind in enumerate(winds):
        for j, cloud in enumerate(clouds):
            ...
            if os.path.exists(img_path):
                ...
                ax.imshow(img)
                ax.set_title(
                    f"Wind: {wind} m/s | Cloud: {cloud}", fontsize=12, fontweight="bold"
                )
            else:
...
def inertial_vs_wind(romsfile: str, drho_crit: float = 0.03):
    ...
    mean_z_local_shear = np.nanmean(mld_shear, axis=1)
    variance_z_local_shear = np.nanvar(mld_shear, axis=1)
    intermittency_factors = variance_z_local_shear / (mean_z_local_shear**2 + 1e-10)

    # coeffiecient_of_variation_squared:
    cv2 = np.nanmean(intermittency_factors)

    # 5. Peak-to-Mean Ratio
    # Mean shear inside the mixed layer
    mean_shear_val = np.nanmean(mld_shear)
    # Peak shear AT the MLD depth (Brannigan 2013)
    peak_shear_val = np.nanmean(
        [shear_mag[t, int(mld_indices[t])] for t in valid_times]
    )
    # peak_shear_val = np.nanmean(np.nanmax(mld_shear, axis=1))
    # ratio = (peak_shear_val - mean_shear_val) / (mean_shear_val + 1e-10)
    ratio = peak_shear_val / (mean_shear_val + 1e-10)

    print("--- Thesis Check: Intermittency Analysis ---")
...
def inertial_vs_wind_old(romsfile: str, drho_crit: float = 0.03):
    ...
    print("--- Thesis Check Calculation (Refined) ---")
        ...
        for cloud_cover in cloud_covers:
            ...
            for experiment_no in [DIURNAL_EXPERIMENT_NO, STABLE_EXPERIMENT_NO]:
                ...
                for name in filenames:
                    ...
                    dst_file = os.path.join(thesis_folder, f"{name}_E{new_id:02}{ext}")
                ...
                for name in extra_files:
                    ...
                    for src_file in files:
                        ...
                        try:
                            ...
                            print(f"Warning: File not on correct form {filename}")
                            continue
                        dst_file = os.path.join(
                            thesis_folder, f"{name}_E{new_id1:02}-E{new_id2:02}{ext}"
                        )
                        shutil.copy2(src_file, dst_file)
                        print(
                            f"Copied: {filename} from ({id1}, {id2}) -> E{new_id1:02}, E{new_id2:02}"
                        )
